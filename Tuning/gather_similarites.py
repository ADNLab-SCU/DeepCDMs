...
def main():
    mol_list = parse_sdf_utils.get_sdf_to_mol('#_final_model/prediction_for_167/20231217/14_sim.sdf')
    inchikey_dict = train_test_split_utils.make_inchikey_dict(mol_list)

    inchikey_dict_keys = inchikey_dict.keys()
    inchikey_lst = []
    distance_matrix_lst = []
    for i in inchikey_dict_keys:
        inchikey_lst.append(i)

    for inchikey in inchikey_lst:
        spectra_for_one_mol = make_spectra_array(inchikey_dict[inchikey])
        distance_matrix = get_similarities(spectra_for_one_mol)
        try:
            distance_matrix_lst.append(distance_matrix[0][1])
        except:
            pass
    
    # cossim = sum(distance_matrix_lst)/len(distance_matrix_lst)
    # print("\n", "distance_result\n\n", cossim, "\n")
    print(distance_matrix_lst)

    # spectra_for_one_mol = make_spectra_array(inchikey_dict['NMCCSGZPBZMJLR-UHFFFAOYSA-N'])
    # distance_matrix = get_similarities(spectra_for_one_mol)
    # print('distance for spectra in NMCCSGZPBZMJLR-UHFFFAOYSA-N', distance_matrix)
